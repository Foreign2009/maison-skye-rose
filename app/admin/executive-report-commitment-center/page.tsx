...
import ExecutiveReportCommitmentCenter            from "@/app/admin/ExecutiveReportCommitmentCenter";
...
import { buildExecutiveReportExecution }          from "@/app/lib/operations/ExecutiveReportExecutionBuilder";
import { buildExecutiveReportCompletion }         from "@/app/lib/operations/ExecutiveReportCompletionBuilder";
import { buildExecutiveReportPublication }        from "@/app/lib/operations/ExecutiveReportPublicationBuilder";
import { buildExecutiveReportDistribution }       from "@/app/lib/operations/ExecutiveReportDistributionBuilder";
import { buildExecutiveReportDelivery }           from "@/app/lib/operations/ExecutiveReportDeliveryBuilder";
import { buildExecutiveReportAcknowledgement }    from "@/app/lib/operations/ExecutiveReportAcknowledgementBuilder";
import { buildExecutiveReportReceipt }            from "@/app/lib/operations/ExecutiveReportReceiptBuilder";
import { buildExecutiveReportConfirmation }       from "@/app/lib/operations/ExecutiveReportConfirmationBuilder";
import { buildExecutiveReportVerification }       from "@/app/lib/operations/ExecutiveReportVerificationBuilder";
import { buildExecutiveReportValidation }         from "@/app/lib/operations/ExecutiveReportValidationBuilder";
import { buildExecutiveReportCertification }      from "@/app/lib/operations/ExecutiveReportCertificationBuilder";
import { buildExecutiveReportAuthorization }      from "@/app/lib/operations/ExecutiveReportAuthorizationBuilder";
import { buildExecutiveReportAuthentication }     from "@/app/lib/operations/ExecutiveReportAuthenticationBuilder";
import { buildExecutiveReportRatification }       from "@/app/lib/operations/ExecutiveReportRatificationBuilder";
import { buildExecutiveReportEndorsement }        from "@/app/lib/operations/ExecutiveReportEndorsementBuilder";
import { buildExecutiveReportAcceptance }         from "@/app/lib/operations/ExecutiveReportAcceptanceBuilder";
import { buildExecutiveReportAdoption }           from "@/app/lib/operations/ExecutiveReportAdoptionBuilder";
import { buildExecutiveReportCommitment }         from "@/app/lib/operations/ExecutiveReportCommitmentBuilder";

export const metadata: Metadata = {
  title:  "Executive Report Commitment Center | Maison Skye & Rose",
...
export default async function ExecutiveReportCommitmentCenterPage() {
  ...
  const execution        = buildExecutiveReportExecution(approval);
  const completion       = buildExecutiveReportCompletion(execution);
  const publication      = buildExecutiveReportPublication(completion);
  const distribution     = buildExecutiveReportDistribution(publication);
  const delivery         = buildExecutiveReportDelivery(distribution);
  const acknowledgement  = buildExecutiveReportAcknowledgement(delivery);
  const receipt          = buildExecutiveReportReceipt(acknowledgement);
  const confirmation     = buildExecutiveReportConfirmation(receipt);
  const verification     = buildExecutiveReportVerification(confirmation);
  const validation       = buildExecutiveReportValidation(verification);
  const certification    = buildExecutiveReportCertification(validation);
  const authorization    = buildExecutiveReportAuthorization(certification);
  const authentication   = buildExecutiveReportAuthentication(authorization);
  const ratification     = buildExecutiveReportRatification(authentication);
  const endorsement      = buildExecutiveReportEndorsement(ratification);
  const acceptance       = buildExecutiveReportAcceptance(endorsement);
  const adoption         = buildExecutiveReportAdoption(acceptance);
  const commitment       = buildExecutiveReportCommitment(adoption);

  return <ExecutiveReportCommitmentCenter commitment={commitment} />;
}
