...
// ── Section 1: Endorsement Overview ──────────────────────────────────────────

function EndorsementOverviewSection({ endorsement }: { endorsement: ExecutiveReportEndorsement }) {
  ...
  return (
    <section>
      ...
      <SectionHeading>Endorsement Overview</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Pipeline-level view of the executive report endorsement stage. Refreshed on every page load.
      </p>
...
// ── Section 2: Executive Endorsement Records ──────────────────────────────────

function ExecutiveEndorsementRecordCard({ entry }: { entry: ExecutiveReportEndorsementEntry }) {
  return (
    <Card>
...
function ExecutiveEndorsementRecordsSection({ endorsement }: { endorsement: ExecutiveReportEndorsement }) {
  if (endorsement.records.length === 0) {
    return (
      <section>
        <SectionLabel>Records</SectionLabel>
        <SectionHeading>Executive Endorsement Records</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          All endorsement records in the executive pipeline.
        </p>
        <Card>
          <p className="text-sm text-[#7b7480]">No endorsement records available.</p>
        </Card>
      </section>
    );
  }
  ...
  return (
    <section>
      ...
      <SectionHeading>Executive Endorsement Records</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        {endorsement.records.length} endorsement {endorsement.records.length === 1 ? "record" : "records"} in the executive pipeline.
      </p>

      <div className="space-y-4">
        {endorsement.records.map((entry, i) => (
          <ExecutiveEndorsementRecordCard key={i} entry={entry} />
        ))}
      </div>
    </section>
  );
...
// ── Section 3: Endorsement Status ─────────────────────────────────────────────

function EndorsementStatusSection({ endorsement }: { endorsement: ExecutiveReportEndorsement }) {
  const endorsed  = endorsement.records.filter(r => r.state === "endorsed").length;
  const endorsing = endorsement.records.filter(r => r.state === "endorsing").length;
  const total     = endorsement.records.length;
  const endorsedPct  = total > 0 ? Math.round((endorsed  / total) * 100) : 0;
  const endorsingPct = total > 0 ? Math.round((endorsing / total) * 100) : 0;

  return (
    <section>
      <SectionLabel>Status</SectionLabel>
      <SectionHeading>Endorsement Status</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Endorsement state distribution at generation time. No data is stored or persisted.
      </p>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card>
          <p className="text-[10px] uppercase tracking-widest text-[#a09aa6]">Endorsed</p>
          <p className="mt-2 text-3xl font-black text-green-700">{endorsedPct}%</p>
          <p className="mt-1 text-[10px] text-[#a09aa6]">{endorsed} of {total} {endorsed === 1 ? "record" : "records"}</p>
        </Card>
        <Card>
          <p className="text-[10px] uppercase tracking-widest text-[#a09aa6]">Endorsing</p>
          <p className="mt-2 text-3xl font-black text-[#a09aa6]">{endorsingPct}%</p>
          <p className="mt-1 text-[10px] text-[#a09aa6]">{endorsing} {endorsing === 1 ? "record" : "records"} in progress</p>
        </Card>
      </div>
    </section>
  );
...
// ── Section 4: Endorsement Timeline ───────────────────────────────────────────

function EndorsementTimelineSection({ endorsement }: { endorsement: ExecutiveReportEndorsement }) {
  if (endorsement.records.length === 0) {
    return (
      <section>
        <SectionLabel>Timeline</SectionLabel>
        <SectionHeading>Endorsement Timeline</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          Chronological view of endorsement entries.
        </p>
        <Card>
          <p className="text-sm text-[#7b7480]">No endorsement entries to display.</p>
        </Card>
      </section>
    );
  }
...
export default function ExecutiveReportEndorsementDashboard({ endorsement }: Props) {
  return (
    <div className="min-h-screen bg-[#f5f1eb]">
      ...
      <div className="mx-auto w-full max-w-[780px] space-y-14 px-6 py-12">
        ...
        <EndorsementOverviewSection endorsement={endorsement} />

        <hr className="border-gray-200" />

        <ExecutiveEndorsementRecordsSection endorsement={endorsement} />

        <hr className="border-gray-200" />

        <EndorsementStatusSection endorsement={endorsement} />

        <hr className="border-gray-200" />

        <EndorsementTimelineSection endorsement={endorsement} />
