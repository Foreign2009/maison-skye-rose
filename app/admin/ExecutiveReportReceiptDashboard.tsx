...
// ── Section 1: Receipt Overview ───────────────────────────────────────────────

function ReceiptOverviewSection({ receipt }: { receipt: ExecutiveReportReceipt }) {
  return (
    <section>
      <SectionLabel>Executive Report Receipt</SectionLabel>
      <SectionHeading>Receipt Overview</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Aggregate view of the executive report receipt stage. Refreshed on every page load.
      </p>
...
// ── Section 2: Receipt Timeline ───────────────────────────────────────────────

function ReceiptTimelineSection({ receipt }: { receipt: ExecutiveReportReceipt }) {
  if (receipt.records.length === 0) {
    return (
      <section>
        <SectionLabel>Timeline</SectionLabel>
        <SectionHeading>Receipt Timeline</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          Chronological view of all receipt records.
        </p>
        <Card>
  ...
  return (
    <section>
      ...
      <SectionHeading>Receipt Timeline</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        {receipt.records.length} record{receipt.records.length === 1 ? "" : "s"} in the receipt timeline.
      </p>
...
// ── Section 3: Receipt Records ────────────────────────────────────────────────

function ReceiptRecordCard({ entry }: { entry: ExecutiveReportReceiptEntry }) {
  return (
    <Card>
...
function ReceiptRecordsSection({ receipt }: { receipt: ExecutiveReportReceipt }) {
  if (receipt.records.length === 0) {
    return (
      <section>
        <SectionLabel>Records</SectionLabel>
        <SectionHeading>Receipt Records</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          Detailed view of every receipt record.
        </p>
        <Card>
          <p className="text-sm text-[#7b7480]">No receipt records available.</p>
        </Card>
  ...
  return (
    <section>
      ...
      <SectionHeading>Receipt Records</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        {receipt.records.length} record{receipt.records.length === 1 ? "" : "s"} in receipt.
        Displayed exactly as received.
      </p>

      <div className="space-y-4">
        {receipt.records.map((entry, i) => (
          <ReceiptRecordCard key={i} entry={entry} />
        ))}
      </div>
    </section>
  );
...
// ── Section 4: Receipt Status ─────────────────────────────────────────────────

function ReceiptStatusSection({ receipt }: { receipt: ExecutiveReportReceipt }) {
  ...
  return (
    <section>
      ...
      <SectionHeading>Receipt Status</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Receipt status indicators at generation time.
      </p>
...
function ReceiptMetadataSection({ receipt }: { receipt: ExecutiveReportReceipt }) {
  return (
    <section>
      ...
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Receipt generation metadata. No data is stored or persisted.
      </p>
...
export default function ExecutiveReportReceiptDashboard({ receipt }: Props) {
  return (
    <div className="min-h-screen bg-[#f5f1eb]">
      ...
      <header className="flex items-center justify-between bg-[#4f4a52] px-6 py-4 print:hidden">
        <div className="flex items-center gap-6">
          ...
          <nav className="flex items-center gap-4">
            ...
            <span className="text-xs font-bold text-white">Executive Report Receipt</span>
      ...
      <div className="mx-auto w-full max-w-[780px] space-y-14 px-6 py-12">
        ...
        <ReceiptOverviewSection receipt={receipt} />
        ...
        <ReceiptRecordsSection receipt={receipt} />

        <hr className="border-gray-200" />

        <ReceiptStatusSection receipt={receipt} />
