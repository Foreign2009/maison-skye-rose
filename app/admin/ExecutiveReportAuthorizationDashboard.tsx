...
// ── Section 1: Authorization Overview ────────────────────────────────────────

function AuthorizationOverviewSection({ authorization }: { authorization: ExecutiveReportAuthorization }) {
  ...
  return (
    <section>
      ...
      <SectionHeading>Authorization Overview</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Authorization pipeline status at generation time. Refreshed on every page load.
      </p>
...
// ── Section 2: Authorization Timeline ────────────────────────────────────────

function AuthorizationTimelineSection({ authorization }: { authorization: ExecutiveReportAuthorization }) {
  if (authorization.records.length === 0) {
    return (
      <section>
        <SectionLabel>Timeline</SectionLabel>
        <SectionHeading>Authorization Timeline</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          Chronological view of authorization entries.
        </p>
        <Card>
  ...
  return (
    <section>
      ...
      <SectionHeading>Authorization Timeline</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        {authorization.records.length} authorization {authorization.records.length === 1 ? "entry" : "entries"} in the pipeline.
      </p>
...
// ── Section 3: Authorization Records ─────────────────────────────────────────

function AuthorizationRecordCard({ entry }: { entry: ExecutiveReportAuthorizationEntry }) {
  return (
    <Card>
...
function AuthorizationRecordsSection({ authorization }: { authorization: ExecutiveReportAuthorization }) {
  if (authorization.records.length === 0) {
    return (
      <section>
        <SectionLabel>Records</SectionLabel>
        <SectionHeading>Authorization Records</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          All authorization records in the current pipeline.
        </p>
        <Card>
          <p className="text-sm text-[#7b7480]">No authorization records available.</p>
        </Card>
      </section>
    );
  }
  ...
  return (
    <section>
      ...
      <SectionHeading>Authorization Records</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        {authorization.records.length} authorization {authorization.records.length === 1 ? "record" : "records"} generated at {fmtDate(authorization.generatedAt)}.
      </p>

      <div className="space-y-4">
        {authorization.records.map((entry, i) => (
          <AuthorizationRecordCard key={i} entry={entry} />
        ))}
      </div>
    </section>
  );
...
// ── Section 4: Authorization Status ──────────────────────────────────────────

function AuthorizationStatusSection({ authorization }: { authorization: ExecutiveReportAuthorization }) {
  const authorized  = authorization.records.filter(r => r.state === "authorized").length;
  const authorizing = authorization.records.filter(r => r.state === "authorizing").length;
  const total       = authorization.records.length;
  const authorizedPct = total > 0 ? Math.round((authorized / total) * 100) : 0;

  return (
    <section>
      <SectionLabel>Status</SectionLabel>
      <SectionHeading>Authorization Status</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Authorization status indicators at generation time. No data is stored or persisted.
      </p>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card>
          <p className="text-[10px] uppercase tracking-widest text-[#a09aa6]">Authorization Rate</p>
          <p className="mt-2 text-3xl font-black text-[#4f4a52]">{authorizedPct}%</p>
...
export default function ExecutiveReportAuthorizationDashboard({ authorization }: Props) {
  return (
    <div className="min-h-screen bg-[#f5f1eb]">
      ...
      <div className="mx-auto w-full max-w-[780px] space-y-14 px-6 py-12">
        ...
        <AuthorizationOverviewSection authorization={authorization} />

        <hr className="border-gray-200" />

        <AuthorizationTimelineSection authorization={authorization} />

        <hr className="border-gray-200" />

        <AuthorizationRecordsSection authorization={authorization} />

        <hr className="border-gray-200" />

        <AuthorizationStatusSection authorization={authorization} />
