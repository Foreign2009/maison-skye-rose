...
// ── Section 1: Ratification Workspace Overview ────────────────────────────────

function RatificationWorkspaceOverviewSection({ ratification }: { ratification: ExecutiveReportRatification }) {
  ...
  return (
    <section>
      ...
      <SectionHeading>Ratification Workspace Overview</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Workspace-level view of the executive report ratification pipeline. Refreshed on every page load.
      </p>
...
// ── Section 2: Executive Ratification Workspace ───────────────────────────────

function ExecutiveRatificationWorkspaceCard({ entry }: { entry: ExecutiveReportRatificationEntry }) {
  return (
    <Card>
...
function ExecutiveRatificationWorkspaceSection({ ratification }: { ratification: ExecutiveReportRatification }) {
  if (ratification.records.length === 0) {
    return (
      <section>
        <SectionLabel>Workspace</SectionLabel>
        <SectionHeading>Executive Ratification Workspace</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          Operational workspace view of all ratification entries.
        </p>
        <Card>
  ...
  return (
    <section>
      ...
      <SectionHeading>Executive Ratification Workspace</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        {ratification.records.length} ratification {ratification.records.length === 1 ? "entry" : "entries"} in the executive workspace.
      </p>

      <div className="space-y-4">
        {ratification.records.map((entry, i) => (
          <ExecutiveRatificationWorkspaceCard key={i} entry={entry} />
        ))}
      </div>
    </section>
  );
...
// ── Section 3: Ratification Review Workspace ──────────────────────────────────

function RatificationReviewWorkspaceSection({ ratification }: { ratification: ExecutiveReportRatification }) {
  if (ratification.records.length === 0) {
    return (
      <section>
        <SectionLabel>Review</SectionLabel>
        <SectionHeading>Ratification Review Workspace</SectionHeading>
        <p className="mt-2 mb-5 text-sm text-[#7b7480]">
          Review workspace for ratification entry comparison.
        </p>
        <Card>
          <p className="text-sm text-[#7b7480]">No ratification entries to review.</p>
        </Card>
      </section>
    );
  }
  ...
  return (
    <section>
      ...
      <SectionHeading>Ratification Review Workspace</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Current and previous headline comparison across all ratification entries.
      </p>
          ...
          {ratification.records.map((entry, i) => (
            <div key={i} className="py-4 first:pt-0 last:pb-0">
              ...
              <p className="mt-1 text-sm text-[#7b7480]">
                {entry.authentication.authorization.certification.validation.verification.confirmation.receipt.acknowledgement.delivery.distribution.publication.completion.execution.approval.decision.action.strategy.outlook.forecast.trend.insight.delta.comparison.previous?.headline.text ?? "—"}
              </p>
            </div>
          ))}
        </div>
...
// ── Section 4: Ratification Readiness ────────────────────────────────────────

function RatificationReadinessSection({ ratification }: { ratification: ExecutiveReportRatification }) {
  const ratified  = ratification.records.filter(r => r.state === "ratified").length;
  const ratifying = ratification.records.filter(r => r.state === "ratifying").length;
  const total     = ratification.records.length;
  const readyPct  = total > 0 ? Math.round((ratified / total) * 100) : 0;

  return (
    <section>
      <SectionLabel>Readiness</SectionLabel>
      <SectionHeading>Ratification Readiness</SectionHeading>
      <p className="mt-2 mb-5 text-sm text-[#7b7480]">
        Ratification readiness indicators at generation time. No data is stored or persisted.
      </p>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card>
          <p className="text-[10px] uppercase tracking-widest text-[#a09aa6]">Ratification Readiness</p>
          <p className="mt-2 text-3xl font-black text-[#4f4a52]">{readyPct}%</p>
          <p className="mt-1 text-[10px] text-[#a09aa6]">{ratified} of {total} ratified</p>
        </Card>
        <Card>
          <p className="text-[10px] uppercase tracking-widest text-[#a09aa6]">Pending Ratification</p>
          <p className="mt-2 text-3xl font-black text-[#4f4a52]">{ratifying}</p>
          <p className="mt-1 text-[10px] text-[#a09aa6]">{ratifying === 1 ? "record" : "records"} ratifying</p>
        </Card>
      </div>
    </section>
  );
...
export default function ExecutiveReportRatificationCenter({ ratification }: Props) {
  return (
    <div className="min-h-screen bg-[#f5f1eb]">
      ...
      <header className="flex items-center justify-between bg-[#4f4a52] px-6 py-4 print:hidden">
        <div className="flex items-center gap-6">
          ...
          <nav className="flex items-center gap-4">
            ...
            <span className="text-xs font-bold text-white">Executive Report Ratification Center</span>
          </nav>
        </div>
      ...
      <div className="mx-auto w-full max-w-[780px] space-y-14 px-6 py-12">
        ...
        <RatificationWorkspaceOverviewSection ratification={ratification} />

        <hr className="border-gray-200" />

        <ExecutiveRatificationWorkspaceSection ratification={ratification} />

        <hr className="border-gray-200" />

        <RatificationReviewWorkspaceSection ratification={ratification} />

        <hr className="border-gray-200" />

        <RatificationReadinessSection ratification={ratification} />
