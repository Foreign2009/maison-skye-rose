        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[99999] flex items-end justify-center bg-black/40 backdrop-blur-sm sm:items-center">
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} transition={{ duration: 0.35 }} className="relative w-full max-w-md overflow-hidden rounded-t-[38px] bg-white p-6 shadow-[0_30px_100px_rgba(0,0,0,0.18)] sm:rounded-[38px]">
            ...
            <div className="relative z-10">
              ...
              <div className="mt-6">
                <p className="mb-3 text-[11px] uppercase tracking-[0.2em] text-[#7b7480]">Quantity</p>
                <div className="flex items-center justify-center gap-4">
                  <motion.button whileTap={{ scale: 0.92 }} onClick={() => setQuantity((prev) => (prev > 1 ? prev - 1 : 1))} className="flex h-12 w-12 items-center justify-center rounded-full bg-[#f6f6f7] text-xl font-bold shadow-sm">-</motion.button>
                  <span className="text-xl font-bold text-[#4f4a52]">{quantity}</span>
                  <motion.button whileTap={{ scale: 0.92 }} onClick={() => setQuantity((prev) => prev + 1)} className="flex h-12 w-12 items-center justify-center rounded-full bg-[#f6f6f7] text-xl font-bold shadow-sm">+</motion.button>
                </div>
              </div>
              <div className="mt-6 rounded-3xl bg-gradient-to-r from-pink-50 to-blue-50 p-5">
