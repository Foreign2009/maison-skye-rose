...
import { wave2Catalogue }        from "./data/wave-2-catalogue";
import { wave3Catalogue }        from "./data/wave-3-catalogue";
...
import { wave8Catalogue }        from "./data/wave-8-catalogue";
...
defaultCatalogueRegistry.register("fragrance", (slug) => {
  ...
  if (wave1) return toFragranceIntake(wave1);
  // Tertiary: Wave 2 staging catalogue (factory-only; not customer-facing).
  // Searched only when production and Wave 1 both return no match.
  const wave2 = wave2Catalogue.find(f => deriveSlug(f.title) === slug);
  if (wave2) return toFragranceIntake(wave2);
  // Quaternary: Wave 3 staging catalogue (factory-only; not customer-facing).
  // Searched only when production, Wave 1, and Wave 2 all return no match.
  const wave3 = wave3Catalogue.find(f => deriveSlug(f.title) === slug);
  if (wave3) return toFragranceIntake(wave3);
  // Quinary: Wave 4 staging catalogue (factory-only; not customer-facing).
  // Searched only when production, Wave 1, Wave 2, and Wave 3 all return no match.
  const wave4 = wave4Catalogue.find(f => deriveSlug(f.title) === slug);
  if (wave4) return toFragranceIntake(wave4);
  // Senary: Wave 5 staging catalogue (factory-only; not customer-facing).
  // Searched only when production, Waves 1–4 all return no match.
  const wave5 = wave5Catalogue.find(f => deriveSlug(f.title) === slug);
  if (wave5) return toFragranceIntake(wave5);
  ...
  if (wave7) return toFragranceIntake(wave7);
  // Nonarary: Wave 8 staging catalogue (factory-only; not customer-facing).
  // Searched only when production, Waves 1–7 all return no match.
  const wave8 = wave8Catalogue.find(f => deriveSlug(f.title) === slug);
  return wave8 ? toFragranceIntake(wave8) : null;
});
